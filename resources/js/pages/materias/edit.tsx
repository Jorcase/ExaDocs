...
export default function Edit({
    ...
    materia: Materia & {
        carreras?: { id: number; nombre: string; pivot?: { anio_sugerido?: number; cuatrimestre?: number } }[];
    ...
    useEffect(() => {
        ...
        planes.forEach((plan) => {
            const careerPivot = carrerasMateria.find((c) => c.id === plan.carrera_id);
            asignaciones.push({
                carrera_id: plan.carrera_id,
                plan_estudio_id: plan.id,
                anio_sugerido: careerPivot?.pivot?.anio_sugerido ?? '',
                cuatrimestre: careerPivot?.pivot?.cuatrimestre ?? '',
            });
        });
        ...
        carrerasMateria.forEach((c) => {
            ...
            if (!yaEsta) {
                asignaciones.push({
                    carrera_id: c.id,
                    plan_estudio_id: '',
                    anio_sugerido: c.pivot?.anio_sugerido ?? '',
                    cuatrimestre: c.pivot?.cuatrimestre ?? '',
                });
            }
        });
