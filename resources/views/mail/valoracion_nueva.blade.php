...
<body>
    <div class="card">
        <div class="header">
            <h1>Nueva valoración en tu archivo</h1>
        </div>
        <div class="content">
            <p>Hola {{ $valoracion->archivo->autor->name ?? 'usuario' }},</p>
            <p>Tu archivo <strong>{{ $valoracion->archivo->titulo ?? 'Archivo' }}</strong> recibió una valoración.</p>
            <p>Puntaje: <strong>{{ $valoracion->puntaje }}/5</strong></p>
            @if($valoracion->comentario)
                <p style="margin: 12px 0; color:#374151; white-space: pre-wrap;">"{{ $valoracion->comentario }}"</p>
            @endif
            <p>Por: {{ $valoracion->autor->name ?? '—' }}</p>
            @php $url = config('app.url') ? rtrim(config('app.url'), '/') . '/archivos/' . ($valoracion->archivo->id ?? '') : '#'; @endphp
