...
<body>
    <div class="card">
        <div className="header">
            <h1>Nuevo comentario en tu archivo</h1>
        </div>
        <div class="content">
            <p>Hola {{ $comentario->archivo->autor->name ?? 'usuario' }},</p>
            <p>Han comentado tu archivo <strong>{{ $comentario->archivo->titulo ?? 'Archivo' }}</strong>.</p>
            <p style="margin: 12px 0; color:#374151; white-space: pre-wrap;">“{{ $comentario->cuerpo }}”</p>
            <p>Por: {{ $comentario->autor->name ?? '—' }}</p>
            @php $url = config('app.url') ? rtrim(config('app.url'), '/') . '/archivos/' . ($comentario->archivo->id ?? '') : '#'; @endphp
