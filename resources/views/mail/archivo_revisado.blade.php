...
<body>
    <div class="card">
        <div class="header">
            <h1>Tu archivo fue revisado</h1>
        </div>
        <div class="content">
            <p>Hola {{ $archivo->autor->name ?? 'usuario' }},</p>
            <p>El archivo <strong>{{ $archivo->titulo }}</strong> fue actualizado al estado <strong>{{ $estadoNuevo }}</strong>.</p>
            <p>Materia: {{ $archivo->materia->nombre ?? '—' }}<br>
               Tipo: {{ $archivo->tipo->nombre ?? '—' }}</p>
            @php $url = config('app.url') ? rtrim(config('app.url'), '/') . '/archivos/' . $archivo->id : '#'; @endphp
